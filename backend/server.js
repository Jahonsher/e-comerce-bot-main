...
async function isBotBlocked(restaurantId) {
  // Restaurant kolleksiyasidan tekshiramiz
...
// ===== RESTAURANT BLOKLASH (superadmin) =====
app.post("/superadmin/block/:restaurantId", superMiddleware, async (req, res) => {
  try {
    ...
    // Admin kolleksiyasini ham yangilaymiz
    await Admin.findOneAndUpdate(
      { restaurantId: req.params.restaurantId, role: { $ne: "superadmin" } },
      { active: !blocked, blockReason: reason || "" }
    );
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
// ===== BIRINCHI ADMIN YARATISH (agar role:admin yo'q bo'lsa) =====
app.post("/admin/create-first", async (req, res) => {
  try {
    const { secret, username, password, restaurantId, restaurantName } = req.body;
    if (secret !== "create-admin-2024") return res.status(403).json({ error: "Ruxsat yo'q" });
    // Faqat role:admin yo'q bo'lsa
    const existing = await Admin.findOne({ role: "admin", restaurantId: restaurantId || "imperial" });
    if (existing) return res.status(400).json({ error: "Bu restaurantda admin allaqachon bor", username: existing.username });
    const hash = await bcrypt.hash(password, 10);
    const admin = await Admin.create({
      username, password: hash,
      restaurantName: restaurantName || "Imperial Restoran",
      restaurantId: restaurantId || "imperial",
      role: "admin", active: true,
      phone: "", address: "", botToken: "", webappUrl: ""
    });
    await ensureRestaurant(admin.restaurantId, admin.restaurantName);
    res.json({ ok: true, username: admin.username, restaurantId: admin.restaurantId });
  } catch(e) {
...
app.put("/superadmin/restaurants/:id", superMiddleware, async (req, res) => {
  try {
    const { password, active, blockReason, subscriptionEnd, ...rest } = req.body;
    const update = { ...rest };
    if (password) update.password = await bcrypt.hash(password, 10);
    if (active !== undefined) update.active = active;
    if (blockReason !== undefined) update.blockReason = blockReason;
    if (subscriptionEnd !== undefined) update.subscriptionEnd = subscriptionEnd;
    ...
    // Restaurant kolleksiyasini ham yangilaymiz
    ...
    await Restaurant.findOneAndUpdate(
      { restaurantId: admin.restaurantId },
      {
        ...
        blocked: isBlocked,
        blockReason: isBlocked ? (blockReason || "Xizmat to'xtatilgan") : ""
      },
      { upsert: true }
    );
